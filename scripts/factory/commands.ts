...
export function upCommands(systemSource: string): readonly Command[] {
  ...
  return [
    ...
    shell("factory", 'context=$(mktemp -d "${TMPDIR:-/tmp}/factory-dev-image.XXXXXX"); trap \'rm -rf "$context"\' EXIT HUP INT TERM; digest=$(./scripts/prepare-dev-image-context.sh . "$context"); image=$(./deploy/local/resolve-dev-image.sh "${FACTORY_DEV_IMAGE:-dev.local/agentic-software-factory-bff}" "$digest"); image_matches_source(){ [ "$(docker image inspect "$image" --format \'{{index .Config.Labels "factory.application/dev-image"}}\' 2>/dev/null || true)" = true ] && [ "$(docker image inspect "$image" --format \'{{index .Config.Labels "factory.application/source-digest"}}\' 2>/dev/null || true)" = "$digest" ]; }; image_matches_source || docker build --label factory.application/dev-image=true --label "factory.application/source-digest=$digest" -t "$image" -f "$context/apps/bff/Dockerfile" "$context"; checksum=$(./deploy/local/secret-checksum.sh factory); FACTORY_SECRETS_CHECKSUM="$checksum" ./deploy/local/rollout-factory.sh "$image" IfNotPresent'),
