/*
 * Unless explicitly acquired and licensed from Licensor under another license, the contents of this file are subject to the Reciprocal Public License ("RPL") Version 1.5, or subsequent versions as allowed by the RPL, and You may not copy or use this file in either source code or executable form, except in compliance with the terms and conditions of the RPL.
 *
 * All software distributed under the RPL is provided strictly on an "AS IS" basis, WITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED, AND LICENSOR HEREBY DISCLAIMS ALL SUCH WARRANTIES, INCLUDING WITHOUT LIMITATION, ANY WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, QUIET ENJOYMENT, OR NON-INFRINGEMENT. See the RPL for specific language governing rights and limitations under the RPL.
 */

import { Elysia } from 'elysia';
import { applicationRoutes } from './application-routes';
import { diagnosticsApiRoutes, userAdministrationApiRoutes } from './diagnostics-auth-routes';
import { implementationRoutes } from './implementation-routes';
import type { InterviewOperationReconciler } from './interview-operations';
import { requirementRoutes } from './requirement-routes';
import type { ServerServices } from './types';

export function factoryApiRoutes(
  services: ServerServices,
  interviewOperations: InterviewOperationReconciler,
  recordError: (request: Request, error: import('../errors').ApplicationError) => void,
) {
  return new Elysia({ name: 'factory-api-routes', normalize: false })
    .use(diagnosticsApiRoutes(services))
    .use(userAdministrationApiRoutes(services))
    .use(applicationRoutes(services))
    .use(requirementRoutes(services, interviewOperations, recordError))
    .use(implementationRoutes(services, recordError));
}
